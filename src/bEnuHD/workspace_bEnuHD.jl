...
struct Workspace
    ...
    function Workspace(N::Int, q::Int, LL::Int, S::Int, T::Int, wt_seq::NTuple,
            samples::Vector{DataSample}, reg::NamedTuple, opt_args::NamedTuple)
            new(N, q, LL, S, T, wt_seq, samples, reg, opt_args)
    end

end

function Workspace(data, wt_seq::NTuple;
    lambdaJE::Real=0.01,
    lambdaHE::Real=0.01,
    ...
    Workspace(N, q, LL, S, T, wt_seq,
            samples, reg, opt_args)

end

function Workspace(s::DataSample, wt_seq::NTuple;
    q::Int = Int(maximum(s.Z)),
    lambdaJE::Real=0.01,
    lambdaHE::Real=0.01,
    weight::Symbol=:counts,
    algorithm ::Symbol = :LD_LBFGS,
...
function Workspace(samples::Vector{DataSample}, wt_seq::NTuple;
    q::Int = Int(maximum(samples[1].Z)),
    lambdaJE::Real=0.01,
    lambdaHE::Real=0.01,
    weight::Symbol=:counts,
    algorithm ::Symbol = :LD_LBFGS,
