...
function optimize(x::Matrix{Float64}, w::Workspace, beta::Vector{Float64}, lNu::Float64; kwargs...)
	...
	optx = @distributed hcat for site = 1:w.N
		...
		opx, opf, status = optimize!( x[:,site], nlopt_fg!(w, site, beta, lNu); kwargs...)
...
function nlopt_fg!(w::Workspace, site::Int, beta::Vector{Float64}, lNu::Float64)
	return function fg!(x::Vector{Float64}, g::Vector{Float64})
		g === nothing && (g = zeros(Float64, length(x)))
		PLsiteAndGrad!(x, g, w, site, beta, lNu)
	end
end

#Nu set of values furnished and not learned
function optimize(x::Matrix{Float64}, w::Workspace, beta::Vector{Float64}, nu::Vector{Float64}; kwargs...)

	vecf = SharedArray{Float64}(w.N)
	fails = SharedArray{Int}(w.N)

	@assert size(x,1) == w.LL

	optx = @distributed hcat for site = 1:w.N

		opx, opf, status = optimize!( x[:,site], nlopt_fg!(w, site, beta, nu); kwargs...)
...
function nlopt_fg!(w::Workspace, site::Int, beta::Vector{Float64}, nu::Vector{Float64})
	return function fg!(x::Vector{Float64}, g::Vector{Float64})
		g === nothing && (g = zeros(Float64, length(x)))
		PLsiteAndGrad!(x, g, w, site, beta, nu)
	end
end
