...
function optimize(x::Matrix{Float64}, w::Workspace, beta::Vector{Float64}, nu::Vector{Float64}, x0::Matrix{Float64}; kwargs...)

	    vecf = SharedArray{Float64}(w.N)
	    fails = SharedArray{Int}(w.N)

        @assert size(x0,1) == w.LL
		@assert size(x,1) == 2*w.LL

	    optx = @distributed hcat for site = 1:w.N

			opx, opf, status = optimize!( x[:,site], nlopt_fg!(w, site, beta, nu, x0[:,site]); kwargs...)
...
function nlopt_fg!(w::Workspace, site::Int, beta::Vector{Float64}, nu::Vector{Float64}, x0::Vector{Float64})
    return function fg!(x::Vector{Float64}, g::Vector{Float64})
		PLsiteAndGrad!(x, g, w, site, beta, nu, x0)
    end
end

#Learning over round0 only

function optimize(x::Matrix{Float64}, w::Workspace; kwargs...)
	    ...
	    optx = @distributed hcat for site = 1:w.N
			...
			opx, opf, status = optimize!( x[:,site], nlopt_fg!(w, site); kwargs...)
...
function nlopt_fg!(w::Workspace, site::Int)
    return function fg!(x::Vector{Float64}, g::Vector{Float64})
		PLsiteAndGrad!(x, g, w, site)
    end
end
